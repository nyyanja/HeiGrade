...
class TranscriptIT extends FacadeIT {
    ...
    @Autowired
    private SpecialityCourseRepository specialityCourseRepository;
    ...
    private ResponseEntity<Transcript> getTranscript(
            String token,
            UUID studentId,
            int level) {
    ...
    void adminShouldGetStudentTranscript() {
       ...
       JExam exam =
        ...
        ResponseEntity<Transcript> response =
                getTranscript(adminToken, student.getId(), 1);
    ...
    void transcriptShouldCalculateWeightedAverage() {
        JCourse firstCourse =
                createCourse("WEIGHT-1", "Weighted Course 1", 6);
        ...
        JCourse secondCourse =
                createCourse("WEIGHT-2", "Weighted Course 2", 54);
        ...
        assertThat(transcript.getGeneralAverage())
   ...
   JExam exam =
        ...
        ResponseEntity<Transcript> response =
                getTranscript(studentToken, student.getId(), 1);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody().getStudentId())
    ...
    void studentShouldNotGetAnotherStudentTranscript() {
        ResponseEntity<Transcript> response =
                getTranscript(studentToken, otherStudent.getId(), 1);
