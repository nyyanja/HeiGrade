...
class TeacherCourseValidatorTest {
  ...
  void setUp() {
    validator =
        new TeacherCourseValidator(userRepository, courseRepository, teacherCourseRepository);
  ...
  void should_reject_null_teacher_course() {
    assertThatThrownBy(() -> validator.accept(null))
        ...
        .hasMessageContaining("the affectation for one course is null it cannot be saved");
  }
  ...
  void should_reject_null_teacher() {
    ...
    assertThatThrownBy(() -> validator.accept(tc))
        .isInstanceOf(ResponseStatusException.class)
        .hasMessageContaining("the teacher affectation for one course is null it cannot be saved");
  }

  @Test
  void should_reject_teacher_without_id() {
    TeacherCourse tc = validTeacherCourse();
    tc.setTeacher(Teacher.builder().build());
  ...
  @Test
  void should_reject_course_without_id() {
    TeacherCourse tc = validTeacherCourse();
    tc.setCourse(Course.builder().build());
  ...
  @Test
  void should_accept_when_existing_is_same_link() {
    TeacherCourse tc = validTeacherCourse();
