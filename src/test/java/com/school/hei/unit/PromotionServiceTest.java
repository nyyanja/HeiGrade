...
class PromotionServiceTest {
  ...
  @Mock private PromotionValidator promotionValidator;
  ...
  void should_find_all_promotions() {
    JPromotion promotion1 =
        JPromotion.builder().id(UUID.randomUUID()).name("L1").year(2024).build();

    JPromotion promotion2 =
        JPromotion.builder().id(UUID.randomUUID()).name("L2").year(2025).build();
  ...
  void should_find_promotion_by_id() {
    ...
    JPromotion entity = JPromotion.builder().id(id).name("L3").year(2026).build();
  ...
  void should_throw_when_promotion_not_found() {
    ...
    assertThrows(ResponseStatusException.class, () -> promotionService.findById(id));
  }

  @Test
  void should_save_promotion() {
    Promotion promotion = Promotion.builder().name("L1").year(2024).build();
    ...
    when(promotionRepository.save(any(JPromotion.class))).thenReturn(savedEntity);
  ...
  void should_update_promotion() {
    ...
    Promotion promotion = Promotion.builder().name("Updated").year(2025).build();

    JPromotion existing = JPromotion.builder().id(id).name("Old").year(2024).build();

    JPromotion updated = JPromotion.builder().id(id).name("Updated").year(2025).build();

    when(promotionRepository.findById(id)).thenReturn(Optional.of(existing));

    when(promotionRepository.save(any(JPromotion.class))).thenReturn(updated);
