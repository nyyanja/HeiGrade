...
public class TeacherCourseValidator implements SaveValidator<TeacherCourse> {
    ...
    public void accept(TeacherCourse teacherCourse) {
        if (teacherCourse == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "the affectation for one course is null it cannot be saved");
        }
        if (teacherCourse.getTeacher() == null || teacherCourse.getTeacher().getId() == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "the teacher affectation for one course is null it cannot be saved");
        }
        if (teacherCourse.getCourse() == null || teacherCourse.getCourse().getId() == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "the course affectation for one course teaching is null it cannot be saved");
        }
        ...
        teacherCourseRepository
                .findByTeacher_IdAndCourse_Id(teacherCourse.getTeacher().getId(), teacherCourse.getCourse().getId())
                .ifPresent(
                        existing -> {
                            boolean isSameLink = teacherCourse.getId() != null && existing.getId().equals(teacherCourse.getId());
                            if (!isSameLink) {
