...
public class StudentGroupHistoryValidator {
    ...
    public void accept(StudentGroupHistory history) {
        ...
        if (history.getStudent() == null) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "student is required");
        }

        if (history.getGroup() == null) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "group is required");
        }

        if (history.getStartDate() == null) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "start date is required");
        }

        if (history.getEndDate() != null
                && history.getEndDate().isBefore(history.getStartDate())) {
            throw new ResponseStatusException(
