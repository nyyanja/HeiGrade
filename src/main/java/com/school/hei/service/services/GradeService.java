...
public class GradeService {
  ...
  public Grade findById(UUID id) {
    ...
    assertCanReadGrade(entity);
    return GradeMapper.toModel(entity);
  }

  public List<Grade> findByExam(UUID examId) {
    requireExistingExam(examId);
    JExam exam = examRepository.findById(examId).orElseThrow();
    if (exam.getCourse() != null) {
      courseAccessService.assertCanAccessCourse(exam.getCourse().getId());
    }
  ...
  public List<Grade> findByStudent(UUID studentId) {
    ...
    if (courseAccessService.isStudent() && !courseAccessService.currentUserId().equals(studentId)) {
      throw new ResponseStatusException(HttpStatus.FORBIDDEN, "not your grades");
    }
    List<JGrade> entities = gradeRepository.findByStudent_Id(studentId);
    return filterGradesForCurrentUser(entities).stream().map(GradeMapper::toModel).toList();
  }
  ...
  public List<Grade> findByGroup(UUID groupId) {
    ...
    List<JGrade> entities = gradeRepository.findByGroupId(groupId);
    return filterGradesForCurrentUser(entities).stream().map(GradeMapper::toModel).toList();
  }
  ...
  public List<Grade> findBySpeciality(UUID specialityId) {
    ...
    List<JGrade> entities = gradeRepository.findBySpecialityId(specialityId);
    return filterGradesForCurrentUser(entities).stream().map(GradeMapper::toModel).toList();
  }
  ...
  public Grade findByStudentAndExam(UUID studentId, UUID examId) {
    ...
    requireExistingExam(examId);
    if (courseAccessService.isStudent() && !courseAccessService.currentUserId().equals(studentId)) {
      throw new ResponseStatusException(HttpStatus.FORBIDDEN, "not your grade");
    }
    JExam exam = examRepository.findById(examId).orElseThrow();
    if (exam.getCourse() != null) {
      courseAccessService.assertCanAccessCourse(exam.getCourse().getId());
  ...
  public List<Grade> findByDate(LocalDate date) {
    ...
    List<JGrade> entities = gradeRepository.findByDate(date);
    return filterGradesForCurrentUser(entities).stream().map(GradeMapper::toModel).toList();
  }
  ...
  public List<Grade> findByMinValue(Double minValue) {
    ...
    List<JGrade> entities = gradeRepository.findByValueGreaterThanEqual(minValue);
    return filterGradesForCurrentUser(entities).stream().map(GradeMapper::toModel).toList();
  }
  ...
  public Grade save(Grade grade) {
    ...
    courseAccessService.assertCanAccessCourse(resolveCourseIdFromGrade(grade));

    if (grade.getDate() == null) {
      grade.setDate(LocalDate.now());
    }
    JGrade saved = gradeRepository.save(GradeMapper.toEntity(grade));
  ...
  public List<Grade> saveAll(List<Grade> grades) {
    ...
    for (Grade grade : grades) {
      ...
      saved.add(GradeMapper.toModel(gradeRepository.save(GradeMapper.toEntity(grade))));
    }
    return saved;
  }
  ...
  private List<JGrade> filterGradesForCurrentUser(List<JGrade> entities) {
          ...
          .filter(
              g ->
                  g.getExam() != null
                      ...
                      && courseIds.contains(g.getExam().getCourse().getId()))
          .toList();
    }
    if (courseAccessService.isStudent()) {
      UUID studentId = courseAccessService.currentUserId();
      return entities.stream()
  ...
  private void assertCanReadGrade(JGrade entity) {
    if (courseAccessService.isAdmin()) {
      return;
    }
    if (courseAccessService.isTeacher()) {
      courseAccessService.assertCanAccessCourse(resolveCourseIdFromExistingGrade(entity));
      return;
    }
    if (courseAccessService.isStudent()) {
      if (entity.getStudent() == null
          || !courseAccessService.currentUserId().equals(entity.getStudent().getId())) {
        throw new ResponseStatusException(HttpStatus.FORBIDDEN, "not your grade");
      }
      return;
    }
  ...
  private UUID resolveCourseIdFromGrade(Grade grade) {
  ...
  private UUID resolveCourseIdFromExistingGrade(JGrade existing) {
    if (existing.getExam() == null
        || existing.getExam().getCourse() == null
        || existing.getExam().getCourse().getId() == null) {
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "grade exam/course is invalid");
    }
    return existing.getExam().getCourse().getId();
  }
